...
surl = "https://ftp.arb-silva.de/release_138/Exports/" \
       "SILVA_138_SSURef_NR99_tax_silva.fasta.gz"
turl = "https://ftp.arb-silva.de/release_138/" \
       "Exports/taxonomy/taxmap_embl-ebi_ena_ssu_ref_nr99_138.txt.gz"
nurl = "https://ftp.ncbi.nih.gov/pub/taxonomy/taxdmp.zip"
print("Downloading " + nurl[nurl.rfind("/") + 1:] + " "
                                                    "may take some time ... ",
      end="", flush=True)
allowedRanks = {"superkingdom": "k__", "phylum": "p__", "class": "c__", "order":
    "o__", "family": "f__", "genus": "g__", "species": "s__"}
...
with zipfile.ZipFile(
        io.BytesIO(urllib.request.urlopen(nurl).read())) as zip_ref:
    with zip_ref.open([name for name in zip_ref.namelist() if
                       os.path.basename(name) == "nodes.dmp"][0]) as zf:
        nodes = {sp(line)[0]: [sp(line)[1], sp(line)[2].decode("UTF-8"), ""] for
                 line in zf}
    with zip_ref.open(
            [name for name in zip_ref.namelist() if name.endswith("names.dmp")][
                0]) as zf:
        for line in zf:
            s = sp(line)
...
with gzip.GzipFile(fileobj=urllib.request.urlopen(turl)) as gzTax, \
    ...
    for line in gzTax:
        ...
        tO.write(sp[0].decode("UTF-8") + "." + sp[1].decode("UTF-8") + "." + sp[
            2].decode("UTF-8") + "\t" + getLineage(sp[5]) + "\n")
print("done")
print("Taxonomy output: " + oname2)
print("Downloading " + surl[surl.rfind("/") + 1:] + " may take some time ... ",
      end="", flush=True)
with gzip.GzipFile(fileobj=urllib.request.urlopen(surl)) as gzSilva, gzip.open(
        oname1, 'wb') as fO:
    for line in gzSilva:
